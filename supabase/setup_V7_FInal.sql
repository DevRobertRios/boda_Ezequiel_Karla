-- =========================================================
-- BODA KARLA & EZEQUIEL — Setup completo de Supabase (UNIFICADO)
-- v7 — invitados por familia + miembros individuales + media
--      + mesas y asientos (plano de mesas, con pos_x/pos_y)
-- =========================================================
-- Este archivo reemplaza a setup_final.sql + alter_mesas.sql.
-- Ya no necesitas correr alter_mesas.sql por separado: las
-- columnas pos_x/pos_y del plano de mesas ya están incluidas
-- aquí desde la creación de la tabla "mesas".
--
-- Cómo usar (instalación desde cero):
...
-- importa perderlos, o si estás montando el proyecto en un
-- Supabase nuevo.
--
-- Si ya tienes datos reales y solo te faltaban las columnas
-- pos_x/pos_y (porque tu base es de antes de esa migración),
-- NO corras este archivo — corre únicamente alter_mesas.sql,
-- que solo agrega esas dos columnas sin borrar nada.
-- =========================================================
...
-- =========================================================
-- TABLA 4: mesas
-- Cada mesa del salón, con su capacidad de asientos y su
-- posición (pos_x, pos_y) dentro del plano visual del admin.
-- =========================================================
create table mesas (
  id          uuid primary key default gen_random_uuid(),
  wedding_id  text not null default 'EK2026',
  ...
  pos_x       integer,             -- posición libre en el plano (antes: alter_mesas.sql)
  pos_y       integer,             -- posición libre en el plano (antes: alter_mesas.sql)
  created_at  timestamptz default now()
);
...
-- desde el panel de admin, o impórtala vía CSV, cuando esté listo
-- =========================================================
